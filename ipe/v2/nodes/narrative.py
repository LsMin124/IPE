...
작문 품질 (BOJ 상용 수준) — 위 규율을 전부 지킨 위에서, 상용 저지(BOJ 등)에 그대로
출제해도 손색없는 완성도를 요구한다:
- 구성: scenario 는 2~4문단으로 짜임새 있게. ① 상황과 등장 주체를 도입하는
  1~3문장(구체적이되 간결 — 배경 설명으로 늘어지지 말 것), ② 주어지는 데이터와
  규칙이 그 상황에서 무엇을 **의미**하는지 서술, ③ 무엇을 구해야 하는지 명확히 묻는
  마무리 — 어떤 값을 계산/출력해야 하는지 **의미 수준으로** 분명하게 끝맺는다
  (입출력 '형식' 서술 금지 규율은 그대로 — 줄 구성·순서·인덱싱은 여전히 쓰지 않는다).
- 문체: 자연스러운 한국어 문어체(…한다/…이다)로 쓴다. 번역투·기계 문체 금지 —
  '당신은 ~할 필요가 있다', '~하는 것이 요구된다' 류. 같은 대상은 처음 붙인 명칭
  하나로 끝까지 일관되게 부른다(용어 드리프트 금지 — '창고'로 시작했으면 중간에
  '물류 거점'으로 바꿔 부르지 않는다). 불필요한 수식어·감탄·과장은 쓰지 않는다.
- 분량: 대략 300~800자. 정보 없는 잡설로 늘리지도, 전보문처럼 압축해 의미 서술을
  누락하지도 말 것 — 모든 문장이 상황·데이터 의미·요구 중 하나에 기여해야 한다.
- 몰입: 도메인의 구체적 상황(작은 이야기)이 데이터의 의미와 자연스럽게 맞물리게
  한다 — 도메인이 장식이 아니라, 입력·규칙·출력이 그 상황에서 당연하게 읽혀야 한다.
  단 도메인 전문지식 없이 이해 가능해야 한다(일반 상식 수준의 용어만 사용).
- 제목: 간결한 명사구(2~6어절)로 시나리오의 핵심 소재를 담는다. 해법·알고리즘 힌트
  금지는 기존 규율 그대로 — 상황을 정확히 가리키는 좋은 제목이 위장의 일부다.
"""
...
작문 품질 (입문 상용 수준): 간결·정확하며 모호함이 없어야 한다. 1~2문단으로, 초등
교과서처럼 평이하고 짧은 문장을 쓴다 — 한 문장에 하나의 사실만. 자연스러운 한국어
문어체(…한다/…구하라)로 쓰고 번역투('당신은 ~할 필요가 있다' 류)는 금지. 같은 대상은
같은 명칭으로 일관되게 부르고, 불필요한 수식어 없이 무엇이 주어지고 무엇을 구하는지만
정확히 서술한다."""
