...
- **계약에 없는 입력 '사용 규칙'을 창작하는 것도 distortion 이다**: 지문이 주어진
  입력의 일부만 유효하다는 필터/자격 조건을 지어내면 왜곡이다 (예: '정점 번호가
  소인수 관계인 전환만 허용', '위상 순서와 일치할 때만 사용 가능' — io_schema/
  불변식 어디에도 없는 규칙). 그런 규칙은 solver 가 입력만으로 검증·재현할 수 없어
  계약의 출력 의미와 지문이 갈라진다 (run v2-54d68df4 실측: QA 5관점 전멸의 근원).
  도메인 서사로 입력의 *의미를 장식*하는 것은 은닉(OK)이나, 입력 데이터의 **유효성
  자체를 바꾸는 규칙 창작**은 reject.
