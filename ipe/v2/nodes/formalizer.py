...
- io_schema 가 허용하는 **퇴화/경계 입력**의 출력 의미를 output_invariants 에
  ...
  되어 QA ambiguity 게이트에서 reject 된다. **복수의 퇴화 규칙이 한 입력에서 겹칠
  수 있으면 우선순위까지 결정**한다 — 예: '시작==끝' 이면서 동시에 '도달 불가/전부
  차단' 인 입력이 constraints 상 가능하면, 어느 규칙이 우선 적용되는지(예: "시작==끝
  이면 다른 조건과 무관하게 0")를 edge_case_semantics 에 명시한다. 우선순위가
  미정의면 겹침 입력에서 구현마다 답이 갈려 QA ambiguity 게이트에서 reject 된다
  (run v2-b4fd4625 실측).
- **답 유일성(tie-break) 결정**: 출력이 선택·정렬·최적화를 거치는 문제는, 동률(tie —
  ...
  게이트에서 reject 된다.
- **제약 크기 설계 (채점 데이터 생성 상한 정합)**: size_range 상한은 실제 채점 데이터
  생성 상한 **이내**에서 의도한 복잡도 분리가 성립하게 잡는다. 생성기의 단일 입력 원소
  총량(배열 원소 N·그래프 간선 E·행렬 R×C·문자열 길이) 상한은 최대 5×10^4 (스트레스
  케이스 생성 상한) — 이보다 큰 상한을 선언해도 실데이터는 그 크기로 만들어지지 않는다.
  의도 복잡도별 권장 상한: O(N²) 정해 의도면 N ≤ 2,000 수준, O(N log N)/O(N) 정해
  의도면 N ≤ 50,000 수준, graph 는 V ≤ 25,000·E ≤ 50,000 수준. **10^5 이상을 선언하지
  말 것** — 생성기가 그 크기를 만들 수 없어 지문 제약과 실제 채점 데이터가 어긋나고,
  지문이 과장한 만큼 naive 오답이 시간 내 통과해 문제의 변별력(품질)이 무너진다.
"""
