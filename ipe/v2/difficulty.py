...
티어 rubric (절대 프레임 — anchor 상대 비교와 **교차 적용**):
- Bronze: 단순 구현·사칙연산·조건문·반복문 수준.
- Silver: 기본 자료구조·정렬·완전탐색·기초 BFS/DFS·기초 수학(소수/GCD).
- Gold: 다익스트라·중급 DP(배낭/비트마스크)·이분탐색 응용·유니온파인드·세그먼트 트리 기초.
- Platinum: 고급 자료구조(레이지 세그, 머지소트 트리)·최대 유량·이분매칭·SCC·2-SAT·HLD.
- Diamond: 고도의 기법 조합(퍼시스턴트/링크컷 트리 등 여러 고급 기법의 비자명한 결합).

추가 규율:
- reasoning 에는 **최소 3개**의 anchor id 를 인용해 상대 위치를 삼각측량하라
  (상회 anchor / 하회 anchor / 동급 anchor 형태가 이상적).
- **복잡도 × n_max 교차확인**: 예) N ≤ 1e5 인데 정해가 O(N^2) 이면 실제 요구
  수준이 낮다는 신호 → 티어 하향 조정.
- 인접한 두 티어 사이에서 애매하면 **보수적으로 낮은 쪽** 티어를 선택하라.

위 rubric 은 좌표계(사전 프레임)로만 쓰고, 최종 판정은 anchor 대비 **상대 위치**로
내린다 — rubric 직관과 anchor 비교가 어긋나면 **anchor 쪽을 우선**하라 (절대 척도
환각 방지).
"""
